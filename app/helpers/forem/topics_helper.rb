module Forem
  module TopicsHelper
    def link_to_latest_post(topic)
      ...
      text = "#{time_ago_in_words(post.created_at)} #{t("ago_by")} #{user.forem_name}"
    ...
    def relevant_posts(topic)
      ...
      if forem_admin_or_moderator?(topic.forum)
        posts
      elsif forem_user.present? && topic.user_id == forem_user.id
        posts.visible.approved_or_pending_review_for(topic.user_id)
      else
        posts.approved
      end
    end
